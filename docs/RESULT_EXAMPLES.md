...
수강생은 자유 주제로 준비한 내 HTML 화면을 공통 연습 폴더(`starter-kits/03-travel-style-test`)의 앱 화면으로 옮기고, 브라우저에서 확인한 뒤 Apps in Toss 콘솔에 올려 QR로 본인 토스 앱에서 확인해요. 아래 네 가지는 이 폴더 구조로 만들 수 있는 앱의 크기와 화면 구성을 보여 주는 완성 예시예요. 유형을 고르는 단계는 없고, 수강생의 주제와 결과물은 각자 준비한 내용에 따라 달라져요.
