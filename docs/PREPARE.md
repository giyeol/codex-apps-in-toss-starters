...
ChatGPT 데스크톱 앱의 Codex를 실행하고 로그인해요. 최신 토스 앱에 본인 명의 계정으로 로그인하고, Apps in Toss 콘솔 가입과 사용할 워크스페이스를 미리 준비해요. QR 테스트는 만 19세 이상이면서 해당 워크스페이스 멤버인 계정에서 진행해야 해요. Git은 필수가 아니에요.

콘솔의 미니앱 등록과 appName 확정은 유형을 선택한 뒤 수업 당일에 진행해요. appName은 코드 설정과 정확히 같아야 하므로 임의로 미리 정하지 않아요.

Apps in Toss 웹 프레임워크(Granite)는 스타터 폴더에서 `pnpm install`할 때 설치돼요. 미리 깔지 마세요.
