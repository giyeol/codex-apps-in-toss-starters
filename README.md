...
4. 전원 같은 폴더 `starter-kits/03-travel-style-test`를 Codex에서 열어요. 폴더를 고르는 단계는 없어요. 할 일은 그 폴더의 `README.md`예요.
...
- `starter-kits/03-travel-style-test/`: 오후에 전원이 여는 공통 연습 폴더
- `starter-kits/`의 나머지와 `complete-examples/`: 앱 크기와 화면 구성을 보는 참고 예시. 수업에서 여는 폴더가 아니에요
- [하루 순서](docs/LEARNER_PATH.md): 오전 공통 HTML → 오후 자유 주제 HTML → 공통 연습 폴더 → QR
- `docs/`: 준비, 참고 예시, 복구, QR 이후 안내
